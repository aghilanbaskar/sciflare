 ...
 useEffect(() => {
  ...
  useEffect(() => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        try {
          e?.preventDefault();
          dispatch(signInUserWithEmailAndPassword({ email, password, organizationId }));
        } catch (error) {
          console.log(error);
      ...
      <div className="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
          <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
              ...
              <form className="space-y-4 md:space-y-6" action="#" onSubmit={handleSubmit}>
                  <div>
                      <Input type="email" name="email" id="email" onChange={(e) => setEmail(e.target.value)} value={email} placeholder="name@company.com" required  label="Email" />
                  </div>
                  <div>
                  <Input type="password" name="password" id="password" onChange={(e) => setPassword(e.target.value)} value={password} placeholder="••••••••" required  label="Password" />
                  </div>
  ...
  <UIModal isOpen={isModalOpen} onClose={closeModal} title="Please select the company to login">
        {
          multipleUsers && multipleUsers.map((user) => {
            return <Card key={user.id} title={user.companyName} onClick={() => {
